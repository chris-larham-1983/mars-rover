describe("STARTUP_SCRIPTS.JS", function() {
    ...
    body.addEventListener("pageshow", function() { responsiveImage(); });
    ...
    describe("declares and initializes various page variables:", function() {
        ...
        it("creates three variables ('availableHeight', 'i', and 'slideshowTimer') that are initialized in the 'responsiveImage()' function", function() {
    ...
    describe("the 'responsiveImage()' function", function() {
        ...
        it("is invoked when the 'pageshow' event fires", function () {
            spy = spyOn(window, "responsiveImage").and.callThrough();
            expect(spy).not.toHaveBeenCalled();
            body.dispatchEvent(pageShowEvent);
            expect(spy).toHaveBeenCalled();
            expect(spy).toHaveBeenCalledTimes(1);
    ...
    describe("the 'bespokeSlideshow()' function", function() {
        ...
        let responsiveImageSpy;
        let bespokeSlideshowSpy;

        beforeEach(function() {
            responsiveImageSpy = spyOn(window, "responsiveImage").and.callThrough();
            index = 10;

            bespokeSlideshowSpy = spyOn(window, "bespokeSlideshow").and.callThrough();
            jasmine.clock().install();
            body.dispatchEvent(pageShowEvent);
            expect(responsiveImageSpy).toHaveBeenCalled();
